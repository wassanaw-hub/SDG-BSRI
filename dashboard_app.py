...
st.markdown('<div class="main-title">📊ระบบรายงานสถิติตัวชี้วัดเป้าหมายการพัฒนาที่ยั่งยืน (SDGs)</div>', unsafe_allow_index=True)
st.markdown('<div class="sub-title">สถาบันวิจัยพฤติกรรมศาสตร์ มหาวิทยาลัยศรีนครินทรวิโรฒ</div>', unsafe_allow_index=True)

# ฟังก์ชันดึงข้อมูลจาก Google Sheets
@st.cache_data
def load_data():
    # ลิงก์ Google Sheets ของคุณ (แปลงเป็นรูปแบบ Export CSV อัตโนมัติ)
    ...
    try:
        ...
        # --- [ส่วนปรับแต่งชื่อคอลัมน์ให้ตรงกับไฟล์จริงของคุณ] ---
        # สมมติฐานโครงสร้างคอลัมน์:
        # 1. คอลัมน์ปี ค.ศ. -> หากชื่ออื่นให้แก้ไขที่นี่ เช่น df.rename(columns={'ปีที่พิมพ์': 'Year'})
        # 2. คอลัมน์ SDG -> เช่น 'SDG', 'เป้าหมาย SDG'
        # 3. คอลัมน์ชื่อบทความ -> เช่น 'ชื่อบทความ', 'Title'
        
        # โค้ดตัวอย่างการทำความสะอาดข้อมูลเบื้องต้น (ตรวจสอบและปรับแก้ตามชื่อคอลัมน์จริง)
        # ตรวจสอบคอลัมน์และตั้งชื่อมาตรฐานเพื่อความง่ายในการเขียนโค้ด
        rename_dict = {}
        ...
        # ตรวจสอบว่ามีคอลัมน์หลักครบไหม ถ้าไม่มีให้สร้างชื่อจำลองไว้ก่อนป้องกันโค้ดพัง
        ...
        # จัดการข้อมูลประเภทข้อความ/ตัวเลข
        ...
        # สร้าง Dataframe จำลองกรณีเชื่อมต่อไม่ได้ เพื่อให้เห็นโครงสร้าง Dashboard
        mock_data = {
...
# จัดการกรณีที่ 1 บทความระบุหลาย SDG แยกด้วยเครื่องหมายจุลภาค (เช่น "SDG 3, SDG 4")
# แตกแถวออกเพื่อให้กราฟนับจำนวนราย SDG ได้ถูกต้อง
df_exploded = df_raw.copy()
df_exploded['SDG_Split'] = df_exploded['SDG'].str.split(',')
df_exploded = df_exploded.explode('SDG_Split')
df_exploded['SDG_Split'] = df_exploded['SDG_Split'].str.strip()
...
if 0 in years_list: years_list.remove(0) # เอาค่าผิดพลาดออก
...
with col2:
    # นับจำนวนบทความที่มีการระบุ SDG สอดคล้อง (ไม่เป็นค่าว่าง หรือ 'ไม่ระบุ')
    matching_sdg_count = df_filtered_raw[
        df_filtered_raw['SDG'].notna() & 
        (df_filtered_raw['SDG'] != '') & 
        (df_filtered_raw['SDG'].lower() != 'ไม่ระบุ') &
        (df_filtered_raw['SDG'].lower() != 'nan')
    ].shape[0]
...
with chart_col2:
    ...
    df_sdg_count = df_sdg_count.sort_values(by='จำนวนบทความ', ascending=True) # เรียงจากมากไปน้อย (ในแนวนอน)
...
# กราฟแท่งเปรียบเทียบแบบไขว้ (Cross-tabulation: SDG แยกตามปี)
